...
export const HomePathRedirection: CanActivateFn = (route, state) => {
    ...
    if(currentPath ==='login'){
      if(user && user.roles && user.roles.includes('Organizer'))  return router.navigate(['/organizer','home']);
      else if(user && user.roles && user.roles.includes('Attendee')) return  router.navigate(['/user','home']);
      else return true;
    }
    if(user && user.roles && user.roles.includes('Organizer') ){
      return router.navigate(['/organizer','home']);
    }
